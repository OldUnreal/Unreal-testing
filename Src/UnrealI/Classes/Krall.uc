...
#exec TEXTURE IMPORT NAME=jkrall FILE=Models\krall.pcx GROUP=Skins DETAIL=Dirty
#exec TEXTURE IMPORT NAME=ekrall FILE=Models\ekrall.pcx GROUP=Skins DETAIL=Dirty
...
function PlayTakeHit(float tweentime, vector HitLoc, int damage)
{
	...
	carc = Spawn(class 'CreatureChunks',,, Location - CollisionHeight * vect(0,0,0.5), Rotation + rotang(16.48,0,90) );
	if (carc != None)
	{
...
function TryToDuck(vector duckDir, bool bReversed)
{
	...
	if ( BonusSkill<=2.f && (Level.TimeSeconds - LastDuckTime < (0.25 + 0.5 * FRand()) * MinDuckTime) )
		return;
	duckDir.Z = 0;
	...
	SetFall();
	if( BonusSkill>0.f && HasAliveEnemy() && !NeedToTurn(Enemy.Location) && FRand()<(BonusSkill*0.25f) )
		PlayRangedAttack();
	else TweenAnim('Jump', 0.3);
...
function PlayWaiting()
{
	...
	if ( bSleeping )
	{
		...
		SetAlertness(-1.0);
		if (AnimSequence == 'Sleep2')
			LoopAnim('Sleep2', AnimSpeed );
		else if (AnimSequence == 'Sleep3')
			LoopAnim('Sleep3', AnimSpeed );
		else
...
function TweenToWaiting(float tweentime)
{
	...
	{
		if (AnimSequence == 'Sleep2')
			TweenAnim('Sleep2', tweentime);
		else if (AnimSequence == 'Sleep3')
			TweenAnim('Sleep3', tweentime);
		else
			TweenAnim('Sleep1', tweentime);
		return;
	}
	if ( bDicePlayer )
	{
...
function PlayRangedAttack()
{
	...
	if( BonusSkill>0.f )
		tweenin = 0.05;
	else if (AnimSequence == 'Shoot1')
		tweenin = 0.3 * FRand();
	else
		tweenin = 0.35;
	PlayAnim('Shoot1', 1.0 - (BonusSkill*0.15), tweenin);
}

function bool IsSameSpeciesAs( ScriptedPawn Other )
{
	return Other.IsA('Krall');
}
