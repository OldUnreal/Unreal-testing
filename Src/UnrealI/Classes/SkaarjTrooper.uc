...
#exec TEXTURE IMPORT NAME=sktrooper1 FILE=Models\zaarj.pcx GROUP=Skins DETAIL=Dirty
...
#exec MESH SOCKET MESH=sktrooper SOCKETNAME="L_Hand" VERT0=200 VERT1=174 VERT2=140 X=1 Y=4 Z=3 PITCH=-28 YAW=1 ROLL=64
#exec MESH SOCKET MESH=sktrooper SOCKETNAME="Head" VERT0=142 VERT1=62 VERT2=63 X=-2 Y=0 Z=-2 PITCH=6 YAW=0 ROLL=0

//-----------------------------------------------------------------------------
// SkaarjTrooper variables.

var() class<weapon> WeaponType;
var	  Weapon myWeapon; // dont spawn SkaarjTroopers on inventory spots
var   float  duckTime;
...
state RangedAttack
{
	...
	function KeepAttacking()
	{
		if ( bFiringPaused )
			return;
		if ( (FRand() > ReFireRate) || (Enemy == None) || (Enemy.Health <= 0)
				|| (Enemy.bDeleteme) || (Enemy == self) || !CanFireAtEnemy() )
		{
			if ( GetAnimGroup(AnimSequence) == 'Shielded' )
			{
				PlayAnim('ShldDown');
