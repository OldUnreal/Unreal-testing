...
#exec MESHMAP SETTEXTURE MESHMAP=Skqueen NUM=1 TEXTURE=Jqueen1 TLOD=10
...
function Scream()
{
	TriggerEvent(ScreamEvent,Self,Instigator);
...
function PlayRangedAttack()
{
	...
	if ( !bJustScreamed && (FRand() < 0.15) )
		Scream();
	else if ( (Shield != None) && (FRand() < 0.5) && (Enemy!=none)
