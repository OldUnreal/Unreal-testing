...
#define CREATE_SHAPE \
	...
	if(Props.Coords) \
		Sh->setLocalPose(NXCoordsToMatrix(*Props.Coords)); \
	rbA->attachShape(*Sh); \
	Sh->release();
...
void UPhysXPhysics::CreateBoxShape(const FShapeProperties& Props, const FVector& Extent)
{
	...
	PRE_INIT_SHAPE;
	physx::PxBoxGeometry Shape(VectToNX3v(Extent * 0.5));
	CREATE_SHAPE;
	POST_CREATE_SHAPE;
	unguard;
}
void UPhysXPhysics::CreateSphereShape(const FShapeProperties& Props, FLOAT Radii)
{
	...
	PRE_INIT_SHAPE;
	physx::PxSphereGeometry Shape(Radii);
	CREATE_SHAPE;
	POST_CREATE_SHAPE;
	unguard;
}
void UPhysXPhysics::CreateCapsuleShape(const FShapeProperties& Props, FLOAT Height, FLOAT Radii)
{
	...
	PRE_INIT_SHAPE;
	physx::PxCapsuleGeometry Shape(Radii, Max((Height * 2), 0.1f));
	CREATE_SHAPE;
	POST_CREATE_SHAPE;
	unguard;
}
void UPhysXPhysics::CreateMeshShape(const FShapeProperties& Props, PX_MeshShape* Mesh, const FVector& Scale)
{
	...
	case PX_MeshShape::RBSHAPE_Tris:
		{
			physx::PxTriangleMeshGeometry Shape(P->GetTriangle(), VectToNX3v(Scale));
			CREATE_SHAPE;
			break;
		}
	case PX_MeshShape::RBSHAPE_Convex:
		{
			physx::PxConvexMeshGeometry Shape(P->GetConvex(), VectToNX3v(Scale));
	...
	case PX_MeshShape::RBSHAPE_MultiConvex:
		{
			...
			{
				physx::PxConvexMeshGeometry Shape(P->ConvexList(i), VectToNX3v(Scale));
				CREATE_SHAPE;
			}
			break;
		}
