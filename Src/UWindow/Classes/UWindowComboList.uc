...
function int FindExactItemIndex(string Value, string Value2, optional bool bIgnoreValueCase, optional bool bIgnoreValue2Case)
{
	...
	{
		if (bIgnoreValueCase && I.Value ~= Value || !bIgnoreValueCase && I.Value == Value)
		{
			if (bIgnoreValue2Case && I.Value2 ~= Value2 || !bIgnoreValue2Case && I.Value2 == Value2)
				return Count;
		}
		++Count;
	}
...
{
	local UWindowComboListItem I, Next;
	...
	while (I != none)
	{
		...
		{
			Next = UWindowComboListItem(I.Next);
			I.Remove();
			I = Next;
		}
		I = UWindowComboListItem(I.Next);
...
function SetSelected(float X, float Y)
{
	...
	Selected = UWindowComboListItem(Items.FindEntry(i));
}
...
{
	if (bCanExecuteItem && Y >= 0 && Y <= WinHeight && Selected != None)
...
{
	Root.CaptureMouse();
	bCanExecuteItem = true;
}

function KeyDown(int Key, float X, float Y)
{
	super.KeyDown(Key, X, Y);
	HandleMouseWheelScrolling(Key, X, Y);
}

function MouseWheelScrolling(int Direction, float X, float Y)
{
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	if (Count > MaxVisible || WinHeight > Root.WinHeight)
	{
		VertSB.ShowWindow();
...
function FocusOtherWindow(UWindowWindow W)
{
	...
	if (!bWindowVisible)
		return;
	if (W.ParentWindow == none || W.ParentWindow.ParentWindow != self && W.ParentWindow != self && W.ParentWindow != Owner)
		CloseUp();
}
