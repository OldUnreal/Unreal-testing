...
#exec TEXTURE IMPORT NAME=MenuTick FILE=Textures\MenuTick.pcx GROUP="Icons" FLAGS=2 MIPS=OFF
#exec TEXTURE IMPORT NAME=MenuDivider FILE=Textures\MenuDivider.pcx GROUP="Icons" MIPS=OFF
#exec TEXTURE IMPORT NAME=MenuSubArrow FILE=Textures\MenuSubArrow.pcx GROUP="Icons" FLAGS=2 MIPS=OFF
...
function PerformSelect(UWindowPulldownMenuItem NewSelected)
{
	...
	{
		if (!bQuiet && Selected != NewSelected && NewSelected.Caption != "-" && !NewSelected.bDisabled)
...
function BeforePaint(Canvas C, float X, float Y)
{
	...
	WinLeft = FClamp(WinLeft, 0, Root.WinWidth - WinWidth);
	WinTop = FClamp(WinTop, 0, Root.WinHeight - WinHeight);
}
...
function KeyDown(int Key, float X, float Y)
{
	...
	case 0x0D: // Enter
		...
		else if (Selected != None && Selected.Caption != "-" && !Selected.bDisabled)
		{
			BeforeExecuteItem(Selected);
			ExecuteItem(Selected);
		}
