...
var bool bAllowsMouseWheelScrolling;
var int MouseWheelScrollingSpeed;
...
function WindowEvent(WinMessage Msg, Canvas C, float X, float Y, int Key)
{
	...
	if (Msg == WM_KeyDown && AllowsMouseWheelScrolling() && Root != none && !Root.bHandledWindowEvent)
	{
...
{
	return bAllowsMouseWheelScrolling || Class.Outer.Name == 'UMenu';
}

function MouseWheelScrolling(int Direction, float X, float Y)
{
	VertSB.Scroll(Direction * MouseWheelScrollingSpeed);
}
