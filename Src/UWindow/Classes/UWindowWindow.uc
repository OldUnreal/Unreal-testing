...
#exec TEXTURE IMPORT NAME=BlackTexture FILE=Textures\Black.pcx MIPS=0
#exec TEXTURE IMPORT NAME=WhiteTexture FILE=Textures\White.pcx MIPS=0
...
var bool				bHandleStoreage;			// Store this component in temp available for rest of the map?
...
const IK_MouseWheelUp = 0xEC;
const IK_MouseWheelDown = 0xED;
const IK_Up = 0x26;
const IK_Down = 0x28;
const IK_PageUp = 0x21;
const IK_PageDown = 0x22;
const IK_Enter = 0x0D;
const IK_Escape = 0x2B;
...
{
	return GetLocalPlayerPawn();
}

final function LevelInfo GetLevel()
{
	return class'LevelInfo'.Static.GetLevelInfo();
}

final function LevelInfo GetEntryLevel()
{
	return GetLocalPlayerPawn().GetEntryLevel();
}
...
function LMouseUp(float X, float Y)
{
	if (bMouseDown)
	{
		if (!bIgnoreLDoubleClick && Abs(X-ClickX) <= 1 && Abs(Y-ClickY) <= 1 && GetLevel().TimeSeconds < ClickTime + 0.600)
		{
			DoubleClick(X, Y);
			ClickTime = 0;
		}
...
// bPage - handle PageUp/PageDown
// X, Y - mouse cursor position
function MouseWheelScrolling(int Direction, float X, float Y); // implemented in subclasses
function KeyboardScrolling(int Direction, bool bPage, float X, float Y); // implemented in subclasses
...
final function UWindowWindow CreateWindow(class<UWindowWindow> WndClass, float X, float Y, float W, float H, optional UWindowWindow OwnerW, optional bool bUnique, optional name ObjectName)
{
	...
	// Launch UWindow if closed.
	if ( !Root.Console.IsInState('UWindow') )
		Root.Console.bPendingOpenConsole = True;

	if (bUnique)
	{
		Child = Root.FindChildWindow(WndClass, True);

		if ( Child==None && WndClass.Default.bHandleStoreage )
			Child = Root.Console.FindOldMenu(WndClass,Self);
		if (Child != None)
		{
			Child.ShowWindow();
			Child.BringToFront();
	...
	// Add it to newly created windows list.
	if ( Child.bHandleStoreage )
		Root.Console.ListUWindowMenu(Child);
...
final function DrawHorizTiledPieces( Canvas C, float DestX, float DestY, float DestW, float DestH, TexRegion T1, TexRegion T2, TexRegion T3, TexRegion T4, TexRegion T5, float Scale )
{
	...
	Pieces[0] = T1;
	if (T1.T != None) PieceCount = 1;
	Pieces[1] = T2;
	if (T2.T != None) PieceCount = 2;
	Pieces[2] = T3;
	if (T3.T != None) PieceCount = 3;
	Pieces[3] = T4;
	if (T4.T != None) PieceCount = 4;
	Pieces[4] = T5;
	if (T5.T != None) PieceCount = 5;
...
final function string RemoveAmpersand(string S)
{
	...
	while (true)
	{
		...
		if (Mid(S, NewPos, 1) == "&")
		{
			...
			S = Left(S, NewPos-1)$Mid(S, NewPos);
			Pos = NewPos+1;
		}
		else
		{
...
final function byte ParseAmpersand(string S, out string Result, out string Underline, bool bCalcUnderline)
{
	...
	while (True)
	{
		...
		{
			// It's a double &, lets add one to the output.
			Result = Result $ Mid(S, Pos, NewPos-Pos);
			if (bCalcUnderline)
				Underline = Underline $ " ";
			Pos = NewPos+1;
		}
		...
		{
			if (HotKey == 0)
				HotKey = Asc(Caps(Mid(S, NewPos, 1)));
			Result = Result $ Mid(S, Pos, NewPos-Pos-1);
...
{
	Text = ReplaceStr(Text,Replace,With);
}
...
// 227: Check if some window is still active, return True if is, else return False.
function bool HasActiveWindow()
{
	local UWindowWindow Child;

	if ( bUWindowActive || bLeaveOnscreen )
		Return True;
...
// Process a console message and override if returned true.
// In order to get this called, set Root.Console.MessageCatcher = this;
function bool OverrideMessage( PlayerReplicationInfo PRI, coerce string Msg, name N )
{
	Return False;
}

/* Notify after mapchange that current window has been removed from temp available menus. */
