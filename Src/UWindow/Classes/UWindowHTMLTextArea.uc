...
function ProcessURL(string URL)
{
	...
	if (StartsWith(URL, "mailto:", true) ||
		StartsWith(URL, "http://", true) ||
		StartsWith(URL, "https://", true) ||
		StartsWith(URL, "ftp://", true) ||
		StartsWith(URL, "telnet://", true) ||
		StartsWith(URL, "gopher://", true))
	{
		GetPlayerOwner().ConsoleCommand("start" @ URL);
	}
	else if (StartsWith(URL, "www.", true))
		GetPlayerOwner().ConsoleCommand("start http://" $ URL);
	else if (StartsWith(URL, "ftp.", true))
		GetPlayerOwner().ConsoleCommand("start ftp://" $ URL);
	else if (StartsWith(URL, "unreal://", true))
		LaunchUnrealURL(URL);
}
...
function UWindowDynamicTextRow AddText(string Text, optional color TxtCol )
{
	local string Output, LeftText, RightText, HTML, Temp;
	...
	while( Len(Text) )
	{
		ParseHTML(Text, LeftText, HTML, RightText);
...
function ParseHTML(const out string Input, out string LeftText, out string HTML, out string RightText)
{
	local int i;
...
final function HTMLUpdateStyle(string Input, out HTMLStyle CurrentStyle)
{
	local string LeftText, HTML, RightText;
...
final function string GetOption(string HTML, const out string Option)
{
	local int i, j;
	local string s;
...
final function int GetHexDigit(const out string D)
{
	local int i;
...
final function SetCanvasStyle(Canvas C, const out HTMLStyle CurrentStyle)
{
	if (CurrentStyle.bLink)
...
final function string WriteStyleText(const out HTMLStyle CurrentStyle, int CharCount)
{
	local string Pad;
	local string Temp;
	local string Output;
...
final function int ReadStyleText(const out string StyleString, out int StylePos, out HTMLStyle CurrentStyle)
{
	local int CharCount;
	local int i;
