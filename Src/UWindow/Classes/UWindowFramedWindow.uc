...
{
	Super.ShowWindow();
	// Make sure this window isnt now off screen.
	if ( WinLeft<0 )
		WinLeft = 0;
	else if ( (WinLeft+WinWidth)>Root.WinWidth )
		WinLeft = Root.WinWidth-WinWidth;
	if ( WinTop<8 ) // Use eight so it dosen't go behind UMenu toolbar.
		WinTop = 8;
	else if ( (WinTop+WinHeight)>Root.WinHeight )
		WinTop = Root.WinHeight-WinHeight;
}

defaultproperties
{
	ClientClass=Class'UWindow.UWindowClientWindow'
	bHandleStoreage=True
}
