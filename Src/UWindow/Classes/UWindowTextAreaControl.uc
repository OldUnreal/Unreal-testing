...
function Paint( Canvas C, float X, float Y )
{
	...
	{
		if ( TextAreaCol[Line].R==0 && TextAreaCol[Line].G==0 && TextAreaCol[Line].B==0 )
			C.DrawColor = White;
		else C.DrawColor = TextAreaCol[Line];
...
function AddText(string NewLine, optional color Col )
{
	TextArea[Head] = NewLine;
	TextAreaCol[Head] = Col;
	Head = (Head + 1)%BufSize;
