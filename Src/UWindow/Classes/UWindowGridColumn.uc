...
function LMouseDown(float X, float Y)
{
	...
	if (X > Min(WinWidth - 5, ParentWindow.WinWidth - WinLeft - 5) && Y < 12)
	{
		...
		UWindowGrid(ParentWindow.ParentWindow).LeftClickRowDown(GetRowIndex(Y), X + WinLeft, Y + WinTop);
}
...
function LMouseUp(float X, float Y)
{
	...
	if (Y > 12)
		UWindowGrid(ParentWindow.ParentWindow).LeftClickRow(GetRowIndex(Y), X + WinLeft, Y + WinTop);
}
...
function MouseMove(float X, float Y)
{
	...
	if (Y > 12)
		UWindowGrid(ParentWindow.ParentWindow).MouseMoveRow(GetRowIndex(Y), X + WinLeft, Y + WinTop, bMouseDown);
}
...
function int GetRowIndex(float Y)
{
	return (Y - 12) / UWindowGrid(ParentWindow.ParentWindow).RowHeight + UWindowGrid(ParentWindow.ParentWindow).TopRow;
}
