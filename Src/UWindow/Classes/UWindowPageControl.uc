...
function UWindowPageControlPage AddPage(string Caption, class<UWindowPageWindow> PageClass, optional name ObjectName)
{
	...
	if (P != SelectedTab)
		P.Page.HideWindow();
	else if (UWindowPageControlPage(SelectedTab) != None && WindowIsVisible())
	{
...
function UWindowPageControlPage InsertPageAfter(UWindowPageControlPage AfterPage, string Caption, class<UWindowPageWindow> PageClass, optional name ObjectName)
{
	local UWindowPageControlPage P;

	if (AfterPage == none)
		P = UWindowPageControlPage(InsertTabAfter(UWindowPageControlPage(Items), Caption));
	else
		P = UWindowPageControlPage(InsertTabAfter(AfterPage, Caption));
	P.Page = UWindowPageWindow(CreateWindow(PageClass, 0,
											TabArea.WinHeight-(LookAndFeel.TabSelectedM.H-LookAndFeel.TabUnselectedM.H),
											WinWidth, WinHeight-(TabArea.WinHeight-(LookAndFeel.TabSelectedM.H-LookAndFeel.TabUnselectedM.H)),,,ObjectName));
	P.Page.OwnerTab = P;
