...
function TextAreaClipText(Canvas C, float DrawX, float DrawY, coerce string S, optional bool bCheckHotkey, optional color Col )
{
	local float X, Y, W, H;
	...
	while (Text != "")
	{
		...
		if (bOverLink && (bMouseDown || bReleased))
		{
			if (bReleased)
			{
				...
				else if ( Left(NextBlock, 9) ~= "unreal://" )
