...
function TextAreaClipText(Canvas C, float DrawX, float DrawY, coerce string S, optional bool bCheckHotkey, optional color OverrideCol)
{
	...
	if ( OverrideCol.R!=0 || OverrideCol.B!=0 || OverrideCol.G!=0 )
	{
		OldC = C.DrawColor;
		C.DrawColor = OverrideCol;
		ClipText(C, DrawX, DrawY, S, bCheckHotkey);
		C.DrawColor = OldC;
	}
	else ClipText(C, DrawX, DrawY, S, bCheckHotkey);
}
...
function UWindowDynamicTextRow AddText( string NewLine, optional color TextCol )
{
	...
	L.WrapParent = None;
	L.bRowDirty = True;
	L.OverrideTextColor = TextCol;

	if (Temp != "")
		AddText(Temp,TextCol);
...
function float DrawTextLine(Canvas C, UWindowDynamicTextRow L, float Y)
{
	...
	if (bHCenter)
	{
		...
		X = 2;
	TextAreaClipText(C, X, Y, L.Text,,L.OverrideTextColor);
