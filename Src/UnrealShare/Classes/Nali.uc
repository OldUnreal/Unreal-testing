...
final function WarnEnemies()
{
	local Pawn P;
	
	foreach VisibleCollidingActors(class'Pawn', P, 1200.f, Location)
		if( P.Health>0 && !P.bIsPlayer && !P.bIsAmbientCreature )
			P.SeePlayer(Self);
}
...
ignores HearNoise, SeePlayer;
	...
	{
		Super.AlarmDone();
		if( BonusSkill>0.f && (BonusSkill>=3.f || FRand()<(BonusSkill*0.35f)) ) // 227j: Higher difficulties have a chance of notifying enemies of Nalis attempt to aid player.
			WarnEnemies();
	}
}
