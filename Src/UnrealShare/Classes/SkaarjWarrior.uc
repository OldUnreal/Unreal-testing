...
#exec TEXTURE IMPORT NAME=Skaarjw1 FILE=Models\Skaarj.pcx GROUP=Skins DETAIL=Pock
#exec TEXTURE IMPORT NAME=Skaarjw4 FILE=Models\Skar2.pcx GROUP=Skins DETAIL=Pock
...
function PostBeginPlay()
{
	...
	if ( skill >= 3 )
	{
		SpinDamage = Min(SpinDamage*1.25,255);
		ClawDamage = Min(ClawDamage*1.25,255);
		LungeDamage = Min(LungeDamage*1.25,255);
	}
	bStealthMelee = (BonusSkill>1.f && !Rand(3));
}
...
function TryToDuck(vector duckDir, bool bReversed)
{
	...
	SetFall();
	if ( duckLeft )
		PlayAnim('LeftDodge', 1.35+(BonusSkill*0.1f));
	else
		PlayAnim('RightDodge', 1.35+(BonusSkill*0.1f));
...
{
	if ( !HasAliveEnemy() || (bStealthMelee && TryToSneakUp() && VSize(Enemy.Location-Location)<1500.f) )
		PlayRunning();
	else Super.PlayCombatMove();
}
...
function bool CanFireAtEnemy()
{
	...
	if( bStealthMelee ) // 227j: Try to melee player from behind if possible.
	{
		if( TryToSneakUp() && VSize(Enemy.Location-Location)<1500.f )
			return false;
		bStealthMelee = false;
	}
